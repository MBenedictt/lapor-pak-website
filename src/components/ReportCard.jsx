import { MessageCircle, Bookmark, Share, Eye, ChevronUp, ChevronDown, MoreVertical, ExternalLink, MapPin } from "lucide-react";
...
const ReportCard = ({ report }) => {
  ...
  return (
    <div className="bg-white rounded-lg overflow-hidden shadow-lg max-w-sm border border-gray-200">
      {/* Header */}
            ...
            <div className="flex items-center space-x-1">
              <button onClick={handleUpvote}>
                <ChevronUp className={`w-4 h-4 cursor-pointer ${
                  userVote === 'up' ? 'text-green-600' : 'text-gray-600 hover:text-green-600'
                }`} />
              </button>
              <span className="text-gray-600 text-sm">{upvotes - downvotes}</span>
              <button onClick={handleDownvote}>
                <ChevronDown className={`w-4 h-4 cursor-pointer ${
                  userVote === 'down' ? 'text-red-600' : 'text-gray-600 hover:text-red-600'
                }`} />
              </button>
            </div>
            <div className="flex items-center space-x-1">
            ...
            <button onClick={handleBookmark}>
              <Bookmark 
                className={`w-4 h-4 cursor-pointer ${
                  isBookmarked ? 'text-yellow-600 fill-yellow-600' : 'text-gray-600 hover:text-yellow-600'
                }`} 
              />
            </button>
